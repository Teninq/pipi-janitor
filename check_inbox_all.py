...
def list_inbox_all(email_user, app_password):
    try:
        mail = imaplib.IMAP4_SSL("imap.gmail.com")
        mail.login(email_user, app_password)
        mail.select("inbox")
        
        # Search for ALL messages in inbox (read or unread)
        ...
        samples = []
        # Get last 20 messages (most recent)
        ...
        mail.logout()
        return {"total_messages_in_inbox": count, "samples": samples}
    except Exception as e:
        return f"Error: {str(e)}"

config = {"email": "lvchenkai0812@gmail.com", "app_password": "suyt jxsp xanh fllc"}
print(json.dumps(list_inbox_all(config["email"], config["app_password"]), ensure_ascii=False, indent=2))
